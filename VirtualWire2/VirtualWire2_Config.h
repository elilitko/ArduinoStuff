#ifndef VirtualWire2_Config_h
#define VirtualWire2_Config_h
//////////////////////////////////////////////////////////////////////////
//
//	Конфигурация использования таймера 2 в Ардуино на ATMega328
//
//
// Константа USE_TIMER2 если опеределена, то на Uno/Nano/Etc будет использован
// таймер/счётчик 2 (вместо 1)
//
#define	USE_TIMER2

// Пин FREQUENCY_CHECK_PIN (будет сконфигурирован на OUTPUT в vw_setup)
// Предназначен для контроля частоты таймера (для отладки)
// Если этот пин определить, то на нём можно проверять частоту таймера.
// При НОРМАЛЬНОЙ РАБОТЕ это пин определять НЕ НАДО
// 
//#define	FREQUENCY_CHECK_PIN 2

//
// Константа REPORT_TIMER_PARAMETERS 
//	(если используется, то надо ОБЯЗАТЕЛЬНО вызвать Serial.begin перед vw_setup !!!)
//	Более точно - здесь не обязательно использовать именно Serial - подойдёт
// любой объект, имеющий методы print и println (например, большиснтво бибилиотек LCD)
//
//	Если определена, то в Serial выводится информация о конфигурации таймера
// Выводится делитель частоты и количество тиков.
// Чтобы программы были совместимы необхоимо, чтобы произведения делителя
// на количество тиков были равны!
//	Например, при скорости 2 kбит/сек (вызов vw_setup(2000);) настройки получаются:
//		с таймером 1
//			Prescaler: 1
//			Ticks : 1000
//		с таймером 2
//			Prescaler: 8
//			Ticks : 125
//	Поскольку 8*125 == 1*1000 - на этой скорости программы будут совместимы
// При НОРМАЛЬНОЙ РАБОТЕ это этого делать НЕ НАДО
//
