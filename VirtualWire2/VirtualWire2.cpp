...
uint8_t vw_timer_calc(uint16_t speed, uint16_t max_ticks, uint16_t *nticks)
{
...
#ifdef USE_TIMER2 
	uint16_t prescalers[] = { 0, 1, 8, 32, 64, 128, 256, 1024, 3333 };
#else	// #ifdef USE_TIMER2
	uint16_t prescalers[] = { 0, 1, 8, 64, 256, 1024, 3333 };
#endif	//	#ifdef USE_TIMER2
	 static const uint8_t TotalPrescalers = sizeof(prescalers) / sizeof(prescalers[0]);
    ...
    // test increasing prescaler (divisor), decreasing ulticks until no overflow
	 for (prescaler = 1; prescaler < TotalPrescalers; prescaler += 1)
    {
    ...
    // Check for error
	 if ((prescaler == (TotalPrescalers - 1)) || (ulticks < 2) || (ulticks > max_ticks))
    {
        // signal fault
        *nticks = 0;
        return 0;
...
#ifdef FREQUENCY_CHECK_PIN
	 pinMode(FREQUENCY_CHECK_PIN, OUTPUT);
#endif // FREQUENCY_CHECK_PIN
...
#ifdef USE_TIMER2 
	 prescaler = vw_timer_calc(speed, (uint8_t)-1, &nticks);
#ifdef REPORT_TIMER_PARAMETERS
	 uint16_t prescalers[] = { 0, 1, 8, 32, 64, 128, 256, 1024 };
#endif	//	REPORT_TIMER_PARAMETERS
#else	// #ifdef USE_TIMER2
	 prescaler = vw_timer_calc(speed, (uint16_t)-1, &nticks);
#ifdef REPORT_TIMER_PARAMETERS
...
#endif	//	#ifdef USE_TIMER2
#ifdef REPORT_TIMER_PARAMETERS
	 REPORT_TIMER_PARAMETERS.print("Prescaler: "); 
	 REPORT_TIMER_PARAMETERS.println(prescalers[prescaler]);
	 REPORT_TIMER_PARAMETERS.print("Ticks: "); 
	 REPORT_TIMER_PARAMETERS.println(nticks);
#endif	//	REPORT_TIMER_PARAMETERS
...
{
#ifdef FREQUENCY_CHECK_PIN
	digitalWrite(FREQUENCY_CHECK_PIN, !digitalRead(FREQUENCY_CHECK_PIN));
#endif // FREQUENCY_CHECK_PIN
